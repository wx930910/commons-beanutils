...
public class PropertyUtilsTestCase extends TestCase {
	 ...
	 * {@code getPropertyDescriptors()}. You should update this list when new
	 ...
	 * (which is fine, according to the JavaBeans spec) but a String setter instead
	 * of a boolean setter.
	 * </p>
	 *
	 * <p>
	 * Although one could logically argue that this combination of method signatures
	 * should not identify a property at all, there is a sentence in Section 8.3.1
	 * making it clear that the behavior tested for here is correct: "If we find
	 * only one of these methods, then we regard it as defining either a read-only
	 * or write-only property called <em>&lt;property-name&gt;</em>.
	 * </p>
	 */
	...
	/**
	 * Positive getPropertyDescriptor on property {@code booleanSecond} that uses an
	 * "is" method as the getter.
	 */
	 ...
	 * {@code properties} should be returned exactly once.
	 */
	...
	/**
	 * Test getting mapped values with slashes in the key. This is different from
	 * periods because slashes are not syntactically significant.
	 */
	...
	/**
	 * Test getting accessible property reader methods for a specified list of
	 * properties that are declared either directly or via implemented interfaces.
	 */
	...
	/**
	 * There is an issue in setNestedProperty/getNestedProperty when the target bean
	 * is a map and the name string requests mapped or indexed operations on a
	 * field. These are not supported for fields of a Map, but it's an easy mistake
	 * to make and this test case ensures that an appropriate exception is thrown
	 * when a user does this.
	 * <p>
	 * The problem is with passing strings of form "a(b)" or "a[3]" to
	 * setNestedProperty or getNestedProperty when the target bean they are applied
	 * to implements Map. These strings are actually requesting "the result of
	 * calling mapped method a on the target object with a parameter of b" or "the
	 * result of calling indexed method a on the target object with a parameter of
	 * 3". And these requests are not valid when the target is a Map as a Map only
	 * supports calling get(fieldName) or put(fieldName), neither of which can be
	 * further indexed with a string or an integer.
	 * <p>
	 * However it is likely that some users will assume that "a[3]" when applied to
	 * a map will be equivalent to (map.get("a"))[3] with the appropriate
	...
	/**
	 * This tests to see that classes that implement Map always have their custom
	 * properties ignored.
	 * <p>
	 * Note that this behavior has changed several times over past releases of
	 * beanutils, breaking backwards compatibility each time. Here's hoping that the
	 ...
	 * change the behavior of setNestedProperty/getNestedProperty when dealing with
	 * objects that implement Map.
	 */
	public void testMapExtensionCustom() throws Exception, IllegalAccessException, IllegalArgumentException,
			InvocationTargetException, IllegalAccessException, NoSuchMethodException, NoSuchMethodException,
			InvocationTargetException, IllegalArgumentException {
		final PropertyUtilsBean utilsBean = PropsFirstPropertyUtilsBean.mockPropertyUtilsBean1();
		final ExtendMapBean bean = new ExtendMapBean();
	 ...
	 * initialized for JDK 1.4+ See BEANUTILS-266 for changes and reason for test
	 */
	...
	/**
	 * Tests whether the default introspection mechanism can be replaced by a custom
	 * BeanIntrospector.
	 */
